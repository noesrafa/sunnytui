...
- **M4.5 (DONE) Minigames** — `ctrl+g` abre `MinigameDialog` con un juego elegido al azar entre snake, 2048 y tetris. `ctrl+n` re-rolea, esc cierra. Cada juego implementa la interface `minigame` (Init/Update/Render/Name); ticks por juego usan un `instance` ID propio para que ticks viejos no muevan partidas nuevas tras re-roll. Snake 30×15, tetris 10×18 con wall kicks básicos, 2048 4×4. Todo en `internal/tui/dialog_minigame.go`.
- **Pendientes** — `?` toggle full-help dialog; modularizar `internal/tui/model.go` (1.6KLOC) cuando crezca la próxima feature grande.
